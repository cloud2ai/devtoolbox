...
class DifyClient:
    ...
    def __init__(self, api_key=None, 
        ...
        if not self.api_key:
            raise ValueError(
                "API key is required for DifyClient. You can set it using "
                "the environment variable 'DIFY_API_KEY'."
            )
        if not self.base_url:
            raise ValueError(
                "Base URL is required for DifyClient. You can set it using "
                "the environment variable 'DIFY_BASE_URL'."
            )
...
class KnowledgeBaseClient(DifyClient):
    ...
    def create_document_by_text(
        ...
        print(f"create_document_by_text data: {data}")
